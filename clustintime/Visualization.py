...
def plot_two_maps(map_1, map_2, title_1, title_2, single_tap=[], multi_tap=[], contrast=1):
    ...
    limit = map_1.shape[0]
    ax1.vlines(single_tap, ymin=0, ymax=limit, linewidth=0.7)
    ax1.vlines(multi_tap, ymin=0, ymax=limit, linewidth=0.7)
    ax1.hlines(single_tap, xmin=0, xmax=limit, linewidth=0.7)  # Same for horizontal
    ax1.hlines(multi_tap, xmin=0, xmax=limit, linewidth=0.7)
    ...
    limit = map_2.shape[0]
    ax2.vlines(single_tap, ymin=0, ymax=limit, linewidth=0.7)
    ax2.vlines(multi_tap, ymin=0, ymax=limit, linewidth=0.7)
    ax2.hlines(single_tap, xmin=0, xmax=limit, linewidth=0.7)  # Same for horizontal
    ax2.hlines(multi_tap, xmin=0, xmax=limit, linewidth=0.7)
