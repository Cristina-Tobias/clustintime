...
def pipeline(data_directory, mask_directory, processing = None, finger_tapping = 'No',  window_size = 1, near = 1, thr = 0.95, contrast = 1, dir_path = '.', TR = 0.5, algorithm = 'infomap', thr_infomap = 0.9, n_clusters = 7, save_maps = 'No', saving_dir = '.'):
    ...
    if processing != None:
        proc.preprocess(corr_map = corr_map, analysis = processing, data = data, window_size = window_size, near = near, thr = thr, contrast = contrast, single_tap = single_tap, multi_tap = multi_tap)    
    if algorithm == 'infomap':
       corr_map, labels = clus.Info_Map(corr_map, thr, multi_tap = multi_tap, single_tap = single_tap)
    elif algorithm == 'KMeans':
       labels = clus.K_Means(corr_map, n_clusters,thr, multi_tap = multi_tap, single_tap = single_tap)
    if save_maps == 'Yes':
